...
| `context` | Frozen canonical external context; public | errors, hard limits | providers, environment | Context values transiently; no stored public hash | Reject duplicate/invalid/oversized values; canonical-vector tests |
...
| `envelope.suites` | Approved suite and context-commitment adapters; internal | models, errors, `_internal.secrets`, cryptographic dependency | CLI, policy parsing, providers, labs | DEK, context-binding key, nonce, plaintext/ciphertext transiently | Uniform auth/context failure; commitment/KAT/negative/limit/zeroization-best-effort tests |
...
| `streaming.service` | Chunk state and bounded seal/open; public service | envelope services, policy, provider protocol, audit, errors, sink protocol | legacy auto-detection, provider SDKs, arbitrary output streams | Plaintext/DEK in bounded buffers | Commit only after final auth/context; abort on every failure; truncation/reorder/cancel tests |
| `streaming.sinks` | Public `TransactionalSink` protocol and state validation | errors, hard limits | envelope parsing, providers, CLI, labs | Uncommitted bounded plaintext/ciphertext writes | Bound writes; one commit; idempotent abort; visibility/state conformance tests |
| `streaming.atomic` | Filesystem `TransactionalSink`; same-directory temp/fsync/link/promotion | policy file rules, sink protocol, errors, OS facade | envelope parsing, providers, labs | Plaintext/ciphertext temporary files | Preserve existing destination; atomic commit/idempotent abort/crash/race/platform tests |
